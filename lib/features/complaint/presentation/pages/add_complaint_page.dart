...
class _AddComplaintPageState extends State<AddComplaintPage> {
      ...
      if (checkInStatus['has_open'] == true) {
        ...
        if (open != null) {
          ...
          if (openDpId != null && openDpId.isNotEmpty) {
            ...
            final openPlanDos =
                doList.where((d) => d.deliveryPlanId == openDpId).toList();
            ...
            if (openCustomerId != null && openCustomerId!.isNotEmpty) {
              ...
              filtered =
                  doList.where((d) => d.customerId == openCustomerId).toList();
  ...
  List<_ComplaintItemRow> _buildItemRows(List<DeliveryOrderModel> selectedDos) {
    ...
    for (final doItem in selectedDos) {
      final items = _itemsByDoId[doItem.id] ?? const <ComplainCreateItemModel>[];
      for (final it in items) {
        final itemId = it.itemId ?? '';
        if (itemId.isEmpty) continue;
        final key = '${doItem.id}_$itemId';
        _qtyReturnCtrls.putIfAbsent(key, () => TextEditingController(text: '1'));
        rows.add(
          _ComplaintItemRow(
            keyId: key,
            doId: doItem.id,
  ...
  Future<void> _submit() async {
    ...
    final selectedDos =
        _doOptions.where((d) => _selectedDoIds.contains(d.id)).toList();
    ...
    try {
      for (final doItem in selectedDos) {
        ...
        await _complainRepo.createComplaint(token: token, data: payload);
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Gagal menyimpan komplain: $e')),
      );
    }
  }
